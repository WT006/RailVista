...
> 更新：2026-09-14
> 相关：走廊清单见 `docs/scenic-railway-lines.md`；**坐标校验与大范围选点策略**见 `docs/scenic-spots-location-verify.md`；演示样例见 `data/presets/z8991.json` 的 `scenicSpots`。
...
**总原则（2026-09-14）：地图钉 = 景点本身在哪里，不是「预计能看到的车窗位置」。**

| 地物类型 | 应存的坐标 | 禁止 |
|---|---|---|
| 具名景点（古城、山峰、景区、工程奇迹等） | **地物本体确切位置**（远眺 / 窗外同一规则） | 高铁站、轨旁望点、臆测观景坐标 |
| 大面积景点（大湖、盆地、平原、戈壁、花海、山脉北麓等） | **本体范围内**选点，可**适当靠近铁路线**（近岸、近缘、穿越带），勿用行政中心/湖心若导致严重飞点 | 为迁就匹配而钉到车站 |
| 列车穿行其上（盐桥、跨河桥、展线） | 结构/轨面真值；`on_track` | 游客中心 |

- `visibility` / `maxDistKm` 只决定**会不会被行程折线筛掉**，**不**改变打点语义。  
- 远眺不够近轨 → 加大 `maxDistKm` 或标 `distant`，**不要**把钉挪到望点。  
- 离轨过远、火车根本看不到 → **不入库**（或工作表「待审」），不要用假望点充数。

细节与校验流程见 **`docs/scenic-spots-location-verify.md`**。
...
- [ ] 坐标为中国境内合理范围；具名点钉本体，大面积点在地貌内可适当靠轨
- [ ] 单线关键出名点已覆盖，总数约 8～20，无灌水 POI
- [ ] 同义重复已合并（保留一个 id）
- [ ] 远眺类已标 `distant`，穿行类已标 `on_track`
- [ ] 未写入 `trainCode` / `at` / `timeLabel` 作为总库必填项

建议后续脚本：对库内点相对已知风景走廊折线算一遍距离，过大且非 `distant` 的给出 warning；完整校验/改点流水线见 `docs/scenic-spots-location-verify.md`。

---

## 7. 与走廊文档的关系

- `docs/scenic-railway-lines.md`：管**哪条铁路**入库（走廊几何）及沿途看点语义（「方向 / 站距 / 穿行」）。
- **本文档**：管**沿线看什么**（风景点库 + 打开车次后的动态过滤）。
- `docs/scenic-spots-location-verify.md`：管**点位是否准确**（本体真值 / 大面积靠轨、外部校验与改点流程）。
