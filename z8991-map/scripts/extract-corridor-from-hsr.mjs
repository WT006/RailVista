...
const HINTS = {
  ...
  // —— 第四期：中原 / 商合杭补网 ——
  zhengfu: [
    '郑州东', '许昌北', '鄢陵南', '扶沟南', '西华', '周口东', '项城', '沈丘北', '界首南', '临泉', '阜阳西',
  ],
  shanghehang: [
    '阜阳西', '颍上北', '霍邱东', '寿县', '淮南南', '水家湖', '合肥南', '巢湖东', '芜湖北', '芜湖',
    '湾沚南', '宣城', '广德南', '安吉', '湖州', '德清', '杭州东',
  ],
  // —— P0 补网（hsr 源足够）——
  jingjin: ['北京南', '亦庄', '永乐', '武清', '天津', '军粮城北', '塘沽', '滨海'],
  huning: [
    '上海', '上海西', '南翔北', '安亭北', '花桥', '昆山南', '阳澄湖', '苏州园区', '苏州', '苏州新区',
    '无锡新区', '无锡', '惠山', '戚墅堰', '常州', '丹阳', '镇江', '宝华山', '南京',
  ],
  husuhu: ['上海虹桥', '上海松江', '练塘', '苏州南', '盛泽', '湖州南浔', '湖州东', '湖州'],
  jiqing: [
    '济南东', '章丘北', '邹平', '淄博北', '临淄北', '青州市北', '潍坊北', '高密北', '胶州北', '青岛',
  ],
  changyichang: ['常德', '汉寿南', '益阳南', '赫山', '长沙西', '长沙'],
  changjiu: ['南昌西', '南昌', '乐化', '永修', '共青城', '德安', '庐山', '九江'],
  panying: ['盘锦', '盘锦北', '海城西', '营口东'],
  jinqin: ['天津', '军粮城北', '滨海', '滨海北', '芦台', '唐山', '滦河', '北戴河', '秦皇岛'],
  // —— P1-A 城际 ——
  chengmianle: [
    '江油', '青莲', '绵阳', '罗江东', '德阳', '广汉北', '青白江东', '新都东', '成都东',
    '成都南', '双流机场', '新津', '眉山东', '夹江', '乐山', '峨眉', '峨眉山',
  ],
  guangzhu: [
    '广州南', '碧江', '北滘', '顺德', '容桂', '南头', '小榄', '东升', '中山北', '中山',
    '南朗', '翠亨', '古镇', '江门', '礼乐', '新会', '古井', '台山', '开平南', '恩平',
    '双水', '珠海北', '唐家湾', '明珠', '前山', '珠海',
  ],
  hanshi: [
    '汉口', '汉川', '天门南', '仙桃西', '潜江', '荆州', '枝江北', '宜昌东',
    '襄阳东', '襄阳', '谷城北', '丹江口', '武当山', '十堰东',
  ],
  ningan: [
    '南京南', '江宁西', '马鞍山东', '当涂东', '芜湖', '弋江', '铜陵北', '铜陵',
    '池州', '九华山', '安庆',
  ],
  changhui: [
    '长春', '龙嘉', '九台南', '双阳', '磐石', '吉林', '蛟河西', '敦化', '大石头南',
    '安图西', '延吉西', '图们北', '珲春',
  ],
  zhengkai: ['郑州东', '绿博园', '贾鲁河', '运粮河', '开封', '宋城路'],
  // —— P2/P3 城际（hsr 源可抽）——
  guangqing: ['花都', '狮岭', '银盏', '清城'],
  guangzhao: ['广州南', '北滘', '顺德北', '佛山西', '三水北', '肇庆东', '肇庆'],
  jingbin: ['北京朝阳', '亦庄', '永乐', '宝坻南', '北辰', '滨海西'],
  jingtang: ['北京城市副中心', '燕郊', '大厂', '香河', '宝坻南', '玉田南', '唐山西', '唐山'],
  jingxiong: ['北京西', '北京大兴', '大兴机场', '固安东', '霸州北', '雄安'],
  weilai: ['潍坊北', '昌邑', '平度西', '莱西北'],
  wugang: ['武汉', '葛店南', '华容南', '黄冈东'],
  wuxian: ['武昌', '汤逊湖', '庙山', '普安', '纸坊东', '土地堂东', '贺胜桥东', '横沟桥东', '咸宁东', '咸宁南'],
  wuxiao: ['汉口', '天河机场', '闵集', '毛陈', '槐荫', '卧龙', '孝感东'],
  zhengji: ['郑州东', '孟庄', '新郑机场'],
  suishen: ['广州东', '新塘', '东莞西', '厚街', '虎门东', '沙田', '滨海湾', '深圳机场北', '深圳机场'],
  // 东环极短：hints 仅 OD，避免错误中间站触发 midFar
  guangzhoudonghuan: ['番禺', '白云机场北'],
};
