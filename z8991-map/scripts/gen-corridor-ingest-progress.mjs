...
const DEFER = {
  ...
  杭衢高铁: 'D：待确认',
  广湛高铁: 'D：待确认',
  哈秦段: 'D：与 jingha 重叠待切片策略',
  // P0 普速剩余
  '陇海—兰新线': 'C：与陇海/兰新分建，不另建超长合并线',
  '同蒲—太焦—焦柳线': 'C：组合线，同蒲已入库；太焦高铁另有 taijiao',
  宁蓉铁路: 'C：graph 无名，待 OSM',
  // P1 名过滤已试、质量不过 → 待 OSM relation
  成渝线: 'C：命名 ways 枢纽 Dijkstra 不通，待 OSM',
  汉丹线: 'C：中段缝隙 >80km，禁 densify 飞线，待 OSM',
  宣杭线: 'C：命名 ways 不连通，待 OSM',
  宝中线: 'C：中段 snap 失败，待 OSM',
  黔桂铁路: 'C：中段缝隙 >90km，待 OSM',
  平齐线: 'C：命名 ways 不连通，待 OSM',
  长图线: 'C：中段缝隙大，待 OSM',
  宁芜线: 'C：末段接芜湖后 medium，待 OSM 整线',
  西平线: 'C：源断口 densify>20km 拒收，待 OSM',
  淮南线: 'C：不到真芜湖（保护锚），待 OSM',
  侯月线: 'C：中段缝隙 >70km，待 OSM',
  黎湛线: 'C：Dijkstra 不通，待 OSM',
  内昆线: 'C：graph 无名，待 OSM',
  遂渝铁路: 'C：graph 无名，待 OSM',
  贵昆线: 'C：graph 几乎无名，待 OSM',
  广梅汕线: 'C：仅梅汕段可过门禁（meishan），全线待 OSM',
  奎北铁路: 'C：中段 ways 不连通，待 OSM',
  北阿铁路: 'C：geometry heavy/回退，待 OSM',
  // P2/P3 探源后仍暂缓
  京滨城际铁路: 'D：hsr/graph 不完整，Dijkstra 不通',
  崇礼铁路: 'D：本地 graph Dijkstra 不通',
  川南城际铁路: 'D：hsr 仅连接线 ~2km',
  佛莞城际铁路: 'D：hsr/graph 无名',
  金建高铁: 'D：本地 graph Dijkstra 不通',
  哈佳铁路: 'D：hsr/graph 无名',
  牡佳高铁: 'D：hsr/graph 无名',
  南龙铁路: 'D：graph 拼出 medium（jump~16km）',
  武石城际: 'D：hsr/graph 无名',
  成蒲铁路: 'D：hsr 无名（成雅含成蒲至朝阳湖）',
  广佛环线: 'D：仅见西环碎片',
  津蓟城际铁路: 'D：hsr/graph 无名',
  琶莲城际铁路: 'D：graph 拼出 medium',
};
